...
function purchaseCard(cardElem, cardImagePath, market) {
    ...
    switchTurn(); // Switch turns after a purchase
}
...
function purchaseLandmark(player, establishment) {
    ...
    // Remove purchased landmark and replenish
...
function rollDicePhase(diceCount) {
    ...
    // After rolling and collecting income, allow player to purchase or skip
    rollDiceButton.style.display = "none"; 
...
class Bot {
    ...
    takeTurn() {
        ...
        const income = collectIncome(diceRoll, this.player, players, true); // Capture the returned income
        const totalIncome = this.player.coins - initialCoins + income; // Total income during the turn
    
        this.logPlayerData(this.player, diceRoll, 'Rolled Dice', null, totalIncome); // Log total income
    ...
    // Get the cheapest card the bot can afford from the market
    getCheapestCard(market) {
        ...
        // Sort by cost and return the cheapest card
