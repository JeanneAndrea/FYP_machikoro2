    ...
    if (establishment.name === 'Business Center') {
        ...
        Array.from(player.hand.children).forEach(cardElem => {
            cardElem.addEventListener('click', function selectPlayerCard() {
                ...
                // Allow selection of opponent's card only after player's card is selected
...
function rollDicePhase(diceCount) {
    ...
    // Cafe-type establishments can activate from opponents' rolls
...
class Bot {
    ...
    logPlayerData(player, diceRoll, action, purchase = null, income = 0) {
        const data = {
            playerId: player.id,
            turnNumber: player.purchaseTurns + 1,
            coins: player.coins,
    ...
    takeTurn() {
        const currentTurnNumber = this.player.purchaseTurns + 1;
    
        if (currentTurnNumber <= this.buyingOnlyTurns) {
            ...
            this.player.purchaseTurns++;
            return;
        }
    
        const rollOneDie = Math.random() < 0.5;
        const diceRoll = rollOneDie ? rollDice() : rollTwoDice();
    
        // Alert Player 2's dice roll
        ...
        this.logPlayerData(this.player, diceRoll, action, purchase);
    ...
    buyCheapestCard() {
        const market = [market1, market2, market3];
        for (let i = 0; i < market.length; i++) {
            const card = this.getCheapestCard(market[i]);
            if (card) {
                card.click(); 
                return card.dataset.imagePath;
            }
        }
        return null; 
    }

    // Get the cheapest card the bot can afford
    getCheapestCard(market) {
        const affordableCards = Array.from(market.children).filter(card => {
            const establishment = establishments.find(est => est.imagePath === card.dataset.imagePath);
            return establishment && this.player.coins >= establishment.cost;
        });
        ...
        return affordableCards.sort((a, b) => {
            const estA = establishments.find(est => est.imagePath === a.dataset.imagePath);
            const estB = establishments.find(est => est.imagePath === b.dataset.imagePath);
            return estA.cost - estB.cost;
        })[0];
    }
}

// Initialize the bot for Player 2
const bot = new Bot(players[1]);
...
// Automated Game Functionality
function startAutomatedGames(numberOfGames) {
    let gameCount = 0;

    function playGame() {
        startGame(); // Initialize game state
        runGameLoop(); // Run one game loop
    }
    ...
    function runGameLoop() {
        ...
        const gameInterval = setInterval(() => {
            ...
            if (currentPlayer.id === 1) {
                if (currentPlayer.purchaseTurns < 3) {
                    purchaseRandomCardOrLandmark();
                } else {
                    simulatePlayerTurn();
                }
    ...
    function simulatePlayerTurn() {
        ...
        rollDicePhase(diceCount);
        purchaseRandomCardOrLandmark();
        switchTurn();
    }

    function purchaseRandomCardOrLandmark() {
        const market = [market1, market2, market3];
        let affordableCards = [];
        ...
        for (let i = 0; i < market.length; i++) {
            affordableCards = affordableCards.concat(
                Array.from(market[i].children).filter(card => {
                    const establishment = establishments.find(est => est.imagePath === card.dataset.imagePath);
                    return establishment && getCurrentPlayer().coins >= establishment.cost;
                })
            );
        }
    ...
    function checkVictory() {
        ...
        if (currentPlayer.landmarks["Launch Pad"]) {
            bot.logPlayerData(currentPlayer, null, 'WIN', 'Won by Launch Pad', 0);
            return true;
        }

        if (Object.values(currentPlayer.landmarks).filter(Boolean).length >= 3) {
            bot.logPlayerData(currentPlayer, null, 'WIN', 'Won by 3 Landmarks', 0);
            return true;
        }
...
// Start the automated games
startAutomatedGames(500);
